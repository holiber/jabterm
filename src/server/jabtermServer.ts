...
export interface JabtermServerOptions {
   ...
   * - bash: injects a `PROMPT_COMMAND` hook
   * - zsh: injects a `precmd` hook via a temporary `ZDOTDIR` `.zshrc`
   *
   * Both emit an OSC marker which the server converts into structured WS messages.
   *
   * Default: false
   */
...
function isZshShell(shellPath: string): boolean {
  const base = shellPath.split(/[\\/]/).pop()?.toLowerCase() ?? shellPath.toLowerCase();
  return base === "zsh" || base.endsWith("zsh");
}
...
function applyShellIntegration(
  ...
  if (isZshShell(shell)) {
    // zsh doesn't support reliable hook injection via env vars alone.
    // Use a temporary ZDOTDIR with a generated .zshrc that sources the user's
    // original rc and appends a precmd hook.
    const origZdotdir = typeof env.ZDOTDIR === "string" && env.ZDOTDIR ? env.ZDOTDIR : null;
    const home = typeof env.HOME === "string" && env.HOME ? env.HOME : null;
    const origRcPath = origZdotdir
      ? path.join(origZdotdir, ".zshrc")
      : home
    ...
    let zdotdir: string;
    try {
      zdotdir = fs.mkdtempSync(path.join(os.tmpdir(), "jabterm-zsh-"));
    } catch {
      return env;
    }

    const sourceOriginal = origRcPath
      ? `[[ -f ${shSingleQuote(origRcPath)} ]] && source ${shSingleQuote(origRcPath)}\n`
      : "";

    const zshrc = `# Generated by jabterm (shellIntegration)
${sourceOriginal}__jabterm_precmd() { printf '\\033]633;D;%s\\007' "$?"; }
typeset -ga precmd_functions
precmd_functions=(__jabterm_precmd $precmd_functions)
`;

    try {
      fs.writeFileSync(path.join(zdotdir, ".zshrc"), zshrc, { encoding: "utf8", mode: 0o600 });
    } catch {
      return env;
    }
